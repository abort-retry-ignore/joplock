...
- **Vault boundary enforcement** (three layers, checked in order):
  1. **Vault ciphertext must never leave vault folders**: if the source is a vault and the destination is not, ciphertext bodies are rejected with 400 (`Vault encrypted data cannot be written outside a vault folder`). This prevents encrypted vault data from leaking into non-vault folders via any code path (UI, proxy, or future callers).
  2. **Vault plaintext move-out requires explicit user confirmation** via `opts.confirmMoveOutOfVault=true`. UI callers pass this flag only after the client has shown a `confirm()` dialog for the vault→non-vault move; without it, a vault-parented plaintext save to a non-vault folder is rejected with 400. This catches accidental folder changes on locked notes (whose body may still be plaintext at rest during transitional `inVault` states) — the note cannot silently escape the vault.
  3. **Policy split via `opts.enforceExistingVault`:**
     - UI callers (`app/routes/api.js` PUT, `app/routes/fragments.js` autosave PUT, `app/routes/history.js` restore) DO NOT set this flag. They rely on `confirmMoveOutOfVault` for explicit user consent.
     - Sync-proxy caller (`app/proxy/vaultProxyGuard.js`) sets `enforceExistingVault: true`. External Joplin sync clients don't understand vaults; if a note currently lives in a vault, they can never push plaintext for it — regardless of what parent_id they claim, ciphertext is required.
  4. **Metadata integrity**: encrypted blobs carry `vault` (folder id) and `noteId` (bound target); both are validated against the destination to prevent cross-vault or cross-note ciphertext smuggling.
- **Client-side lock guard** (`public/app.js`): the editor folder-change handler refuses to move any note whose editor is still showing the locked overlay (`#editor-locked` present) or whose textarea still holds ciphertext. The user is shown "Unlock this note before moving it." No PUT is fired. This is the primary defence; the server-side `confirmMoveOutOfVault` check is defence in depth.
...
- **Vault → non-vault move save fix**: moving an unlocked note from a vault folder to a plain folder was rejected server-side because `assertVaultNoteBodyEncrypted` treated any note whose *existing* parent was a vault as requiring ciphertext, even when the destination was plain. The autosave PUT returned 400, which surfaced in the UI as the offline/disconnected overlay. Fixed by splitting the policy: UI writes only require ciphertext when the DESTINATION is a vault (see the "Encrypted-save identity guard" section); the sync-proxy path opts back into the stricter existing-vault rule via `opts.enforceExistingVault=true` so external Joplin clients still can't strip ciphertext by re-parenting.

- **Vault chrome refresh on folder change**: moving a note between vault and non-vault folders via the editor's `#editor-folder-select` now updates the editor's vault chrome (`#lock-toggle-btn`) in place without a page refresh. Previously `_updateLockToggle`/`_updateNoteLockIcon` only mutated existing DOM, so a plain note moved into a vault got encrypted+saved correctly but never showed the lock toggle button (the button is only rendered by `editorFragment` when the note is already in a vault at SSR time). Fix: `_syncEditorVaultChrome(noteId, inVault, unlocked)` in `public/app.js` injects the button into `.editor-titlebar` (before the delete button) when moving into a vault and removes it when moving out. Called from `_doEncryptNoteInVault` (plain → vault) and the vault-out branch of the folder-change handler. The `_updateLockToggle` helper is kept for the in-place icon swap when the button already exists.
