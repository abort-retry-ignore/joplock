...
<body class="app-shell theme-${escapeHtml(settings.theme || 'matrix')}${settings.noteMonospace ? ' note-body-monospace' : ''}" style="--font-size-note:${escapeHtml(settings.noteFontSize || 15)}px;--font-size-note-mobile:${escapeHtml(settings.mobileNoteFontSize || ((settings.noteFontSize || 15) + 2))}px;--font-size-code:${escapeHtml(settings.codeFontSize || 12)}px;">
	...
	document.body.addEventListener('htmx:afterSwap',function(e){var target=e.detail&&e.detail.target;_log('htmx:afterSwap',target&&target.id);if(target&&target.id==='editor-panel'){if(_cmView){_cmView.destroy();_cmView=null}_searchMarks=[];_searchMarkIdx=0}});
	...
	(function(){function enableLiveSearch(){var el=document.getElementById('nav-search');if(!el||!window.joplockLiveSearch||el.dataset.liveSearch)return;el.dataset.liveSearch='1';el.setAttribute('hx-trigger','search-submit, input changed delay:300ms');el.addEventListener('htmx:beforeRequest',function(e){var v=el.value;if(v.length>0&&v.length<3){e.preventDefault()}});htmx.process(el)}enableLiveSearch();document.body.addEventListener('htmx:afterSettle',enableLiveSearch)})();
	...
	(function(){
		...
		// Update editor title when editor loads
			...
			if(t&&t.id==='mobile-editor-body'){
				...
				if(isMobile()&&window._mobileNewNoteId&&noteId===window._mobileNewNoteId){setTimeout(function(){setEditorMode(_defaultNoteOpenMode==='markdown'?'markdown':'preview')},0)}else{setTimeout(function(){setEditorMode(_defaultNoteOpenMode==='markdown'?'markdown':'preview')},0)}
