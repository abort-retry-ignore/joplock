...
function _setTinyMCEContent(html){
	...
	try{
		...
		setTimeout(function(){
			...
			if(_pendingSearchHighlight){var _bd=_tinymceEditor&&_tinymceEditor.getBody&&_tinymceEditor.getBody();var _hasText=!!(_bd&&(_bd.textContent||'').trim());var _term=activeSearchTerm();if(_term&&_term.trim()&&_hasText){_pendingSearchHighlight=false;applySearchHighlight()}else if(!(_term&&_term.trim())){_pendingSearchHighlight=false}}
		},0);
	}
}
...
document.addEventListener('keydown',function(e){if(e.key==='Escape'){var resViewer=document.getElementById('resource-viewer');if(resViewer&&!resViewer.hidden){_closeResourceViewer();return}var codeModal=document.getElementById('code-modal');if(codeModal&&!codeModal.hidden){closeCodeModal();return}closeFolderContextMenu();closeFolderModal();closeLinkModal();closeNewFolderModal();closeVaultModal();closeHistoryModal();closeEmptyTrashModal();var bar=document.getElementById('search-nav-bar');if(bar&&!bar.hidden){searchNavDismiss();return}if(_searchSessionActive()){searchNavDismiss();return}var navSearch=document.getElementById('nav-search');if(navSearch&&navSearch.value){navSearch.value='';htmx.trigger(navSearch,'search-submit');return}}if(!getTA()&&!getPV()&&!getCM())return;if((e.ctrlKey||e.metaKey)&&!e.altKey&&e.code==='Space'){e.preventDefault();requestManualProseCompletion();return}if((e.ctrlKey||e.metaKey)&&e.key==='b'){e.preventDefault();wrapSel('**','**')}if((e.ctrlKey||e.metaKey)&&e.key==='i'){e.preventDefault();wrapSel('*','*')}if((e.ctrlKey||e.metaKey)&&e.key==='f'){e.preventDefault();if(_editorMode==='preview'&&_searchMarks.length){searchNavStep(1)}else{applySearchHighlight()}}});
...
function maybeHighlightOpenedNote(term){term=(term||'').trim();if(!term)return;var form=activeEditorForm();if(!form||form.dataset.encrypted==='1')return;var si=activeSearchInput();if(si&&!si.value)si.value=term;if(_editorMode==='markdown'||_editorMode==='md'){setTimeout(applySearchHighlight,0);return}/* rich: wait for TinyMCE body text, then highlight */_pendingSearchHighlight=true;var tries=0;var iv=setInterval(function(){tries++;if(!_pendingSearchHighlight||tries>40){clearInterval(iv);return}var b=_tinymceEditor&&_tinymceEditor.getBody&&_tinymceEditor.getBody();if(b&&(b.textContent||'').trim()&&activeSearchTerm()&&activeSearchTerm().trim()){_pendingSearchHighlight=false;clearInterval(iv);applySearchHighlight()}},50)}
